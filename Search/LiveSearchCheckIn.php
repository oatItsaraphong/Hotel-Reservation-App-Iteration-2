...
<?php
    ...
    if(isset($term)){
        ...
        $sql ="SELECT GuestName, ReservationID, Statue, ReservedForGuest, CheckInDate, CheckOutDate, ReservedRoom From GuestTable, ReservationTable WHERE ReservedForGuest = (SELECT GuestIDNum FROM GuestTable WHERE GuestName LIKE '".$term."%' ) AND GuestName LIKE '".$term."%'";
        /*
        $sql =" SELECT GuestName, ReservationID, Statue, ReservedForGuest, CheckInDate, CheckOutDate, ReservedRoom From GuestTable, ReservationTable 
            WHERE (GuestIDNum IN (SELECT ReservedForGuest 
                                FROM ReservationTable
                                 WHERE GuestName LIKE '".$term."%'". 
            "AND Statue = 'Check In')) AND GuestName LIKE '".$term."%'".
            "AND Statue = 'Check In' "; 
